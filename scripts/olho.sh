...
mkdir -p "$OUT_DIR"
NOME="${1:-frame}"
# hflip+vflip gira 180 graus: a webcam está apontada com o aparelho de cabeça para baixo.
"$FFMPEG" -hide_banner -loglevel error -f dshow -video_size 1920x1080 -i video="$CAM" \
  -frames:v 1 -vf "hflip,vflip,rotate=PI" -y "$OUT_DIR/$NOME.jpg" 2>&1 | tail -3
